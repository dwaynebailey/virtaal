...
description: How Dwayne and Claude collaboratively test a PR's real GUI behaviour before merge - write a TESTPLAN-<PR#>.md, Dwayne fills it in live against a running build, Claude triages the findings into fixes. Load before offering to test a PR manually, or when asked to "create a testplan" / "setup for local review" / "cleanup old testplans".
---
...
`shared-checkout-awareness` first - Dwayne may already have something
checked out for his own purposes). Run the relevant test suite once as a
baseline before handing it over, and confirm no stray Virtaal process is
already running (`pgrep -fl "bin/virtaal"`).

## While Dwayne is testing

He fills in the checkboxes himself (`[x]`/`[ ]`) and adds free-text notes
...
**Don't edit the file while he's actively using it** - a `<system-reminder>`
noting it changed on disk mid-session means it's his edit, not a sign of a
sync problem. Wait for an explicit "testplan completed" or similar before
triaging.

## Triaging results

Once he says it's done, read through every `RDB:` note. Not all of them
...
Dwayne's own explicit "cleanup any testplans now complete" instruction,
2026-09-15). Don't delete one that's still mid-review or has unresolved
