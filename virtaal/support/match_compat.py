...
Confirmed live, 2026-08-25: the "Current File" TM plugin
(virtaal/plugins/tm/models/currentfile.py, the only caller of the plain
matcher class in this codebase - terminologymatcher, the other
subclass, overrides matches() with its own non-heap implementation and
isn't affected) crashed with exactly this traceback querying a real
file with two equal-similarity candidate units. Reproducible any time
two or more candidates tie on similarity, not an edge case specific to
one file - short/simple source strings make ties routine. Confirmed
this is still present in translate-toolkit's own latest upstream
master too, not just the pinned version here.
