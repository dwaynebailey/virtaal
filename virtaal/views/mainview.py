...
class MainView(BaseView):
    ...
    def __init__(self, controller):
        ...
        if sys.platform == 'darwin':
            ...
            # included) already derive their colours from this setting, and
            # virtaal.views.theme adapts its own palette to whatever colours
            # the active theme reports - so this is all that is needed for
            # correct GTK3 theming on macOS. There is no rc-based "Leopard
            # theme" step here any more: that was a GTK2-only mechanism
            # (Gtk.rc_parse) emulating OS X 10.5's look, and neither the API
            # nor the look is relevant to a GTK3 app on a modern macOS.
            try:
