...
class MainView(BaseView):
    ...
    def set_saveable(self, value):
        ...
        if value and self.modified:
            return
        if not getattr(self, '_store_savable', True):
    ...
    def _on_store_closed(self, store_controller):
        ...
        self.hide_readonly_notice()
        self._store_savable = True

    def _on_store_loaded(self, store_controller):
        self._store_savable = can_serialize(store_controller.store._trans_store)
        if not self._store_savable:
        ...
        self.gui.get_object('mnu_saveas').set_sensitive(self._store_savable)
