...
class MainView(BaseView):
    ...
    def __init__(self, controller):
        ...
        self.gui.get_object('mnu_show_logs').connect('activate', self._on_show_logs)
    ...
    def _on_show_logs(self, _widget=None):
        # Only the frozen Windows build currently redirects stdout/
        # stderr to these files (pan_app.py) - a dev run or a frozen
        # macOS/Linux build has nothing here yet.
        ...
        if not found:
            dlg = Gtk.MessageDialog(
                ...
                text=_("No log file found"),
            )
            dlg.format_secondary_text(
                _("Nothing has been logged to a file yet. Run Virtaal with "
                  "--debug and --log <filename> to capture one.")
            )
            dlg.run()
            dlg.destroy()
            return
