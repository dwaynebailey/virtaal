...
def test_select_item_finds_a_row_that_is_not_the_first():
    ...
    # name, "B" is the second of the two rows _make_view() creates.
    sview = _make_view()
...
def test_select_item_moves_the_keyboard_cursor_too():
    # select_iter() alone doesn't move the keyboard cursor - Space/
    # Enter act on get_cursor()'s row, not the selection.
    sview = _make_view()
