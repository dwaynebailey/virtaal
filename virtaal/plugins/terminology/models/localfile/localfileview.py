...
class LocalFileView:
    ...
    def _on_populate_popup(self, textbox, menu):
        ...
        if len(selection) > 40:
            #l10n: The context menu entry when adding a term from a very long selection of text. Here start and end are snippets from the start and end of the long selection.
            label = _('Add Term "%(start)s … %(end)s"') % {'start': selection[:15], 'end': selection[-15:]}
        else:
