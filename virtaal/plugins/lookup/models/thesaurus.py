...
to parse, measured directly, not estimated). Once parsed, a locale's
...
class LookupModel(BaseLookupModel):
    ...
    def _check(self, locale_code):
        """Whether a thesaurus exists for locale_code at all - a light
        tree + xcu fetch, not the (possibly large) .dat file itself.
        Kept separate from the download so a locale with genuinely no
        thesaurus can be recognised as such, instead of offering a
        "Download" button that would only ever fail."""
        try:
    ...
    def _download(self, locale_code, folder, files):
        # Only the .dat matters - mythes.py never reads the
        # accompanying .idx (see this module's own docstring), and a
        # real folder (fr_FR, for one) can have a .dat with no .idx
        # anywhere at all. download_dictionary() requires every listed
        # file to succeed, which would abort a genuinely working
        # download over a file nothing here even uses.
    ...
    def _on_parse_failed(self, locale_code):
        # Don't keep retrying a cached .dat that's just broken (a
        # partial/corrupt download) - a real "Download" button is
        # still offered, since a fresh copy might well parse fine.
