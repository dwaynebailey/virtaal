...
def test_create_menu_items_starts_a_background_parse_when_a_dat_is_cached_but_not_yet_parsed(monkeypatch, tmp_path):
    # Parsing a large real thesaurus is real work (English's: ~250-340ms,
    # measured directly) - never done inline while building a context menu.
    monkeypatch.setattr(thesaurus_module, 'thesaurus_cache_dir', lambda: str(tmp_path))
...
def test_on_parse_failed_stops_retrying_the_same_cached_dat():
    # Without this, a broken/partial cached .dat would just get
    # re-parsed - and fail again - on every single right-click.
    model = _make_model()
    model._parsing.add('pl_PL')
...
def test_download_writes_only_dat_files_ignoring_a_missing_idx(monkeypatch, tmp_path):
    # A real case (fr_FR): the .dat exists but no .idx does, anywhere.
    # Nothing here reads the .idx (see mythes.py) - requiring it to
    # exist too would abort a genuinely working download.
    monkeypatch.setattr(thesaurus_module, 'thesaurus_cache_dir', lambda: str(tmp_path))
    fetched = []
...
# Proactive downloads on language change (#3550 follow-up: don't make
# the first right-click the trigger for a download the user never
# asked for - by the time anyone looks a word up, it should usually
# already be there).
